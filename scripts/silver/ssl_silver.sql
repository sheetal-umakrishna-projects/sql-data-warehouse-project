...
CREATE TABLE silver_crm_prod_info (
	prd_id INT,
    cat_id VARCHAR(50),   -- Updated newely for this silver table after data transformation
	prd_key VARCHAR(50),
	prd_nm VARCHAR(50),
    prd_cost INT,
    prd_line VARCHAR(50),
    prd_start_dt DATE,    -- Updated only in silver table after transformation
    prd_end_dt DATE,      -- Updated only in silver table after transformation     
    dwh_create_date DATETIME DEFAULT CURRENT_TIMESTAMP
);
